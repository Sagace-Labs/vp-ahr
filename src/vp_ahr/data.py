"""The AhR dataset: obtain, standardise, verify.
...
TABLE_PATH = DATA_DIR / "ahr_tox21.parquet"
EXAMPLE_PATH = DATA_DIR / "example" / "ahr_example.parquet"
...
def _missing(path: Path) -> str:
    return (
        ...
        "`python -m vp_ahr.data fetch` to rebuild it from PubChem."
    )
...
def _to_compounds(records: pd.DataFrame, structures: pd.DataFrame) -> pd.DataFrame:
    """Collapse receptor-activation records to one row per standardised compound.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m vp_ahr.data")
